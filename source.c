...
#include <string.h> 
#include <time.h>
#include "cful.h" //입력한 날짜에 색을 넣기 위한 외부 헤더파일 참조

void DateToArr(char* date_num, int date); //입력 받은 날짜를 데이터 명으로 저장 
void clear_buffer(void); //출력 버퍼를 비움
void memo(char* file_name); //메모장 실행
void Calendar(int year, int month, int day); //달력 실행
long totalday(int y, int m, int d); //입력 받은 날짜까지의 총 일수를 구함
int isyoon(int a); //윤년 판단 함수
void TodayMemo(void); //오늘의 일정 출력 함수
